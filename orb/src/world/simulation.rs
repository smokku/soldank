...
pub(crate) struct Simulation<
    WorldType: World, /*, const INITIALIZATION_TYPE: InitializationType*/
> {
    world: WorldType,
    ...
    Simulation<WorldType /*, INITIALIZATION_TYPE*/>
{
    ...
    pub fn new(initialization_type: InitializationType) -> Self {
        Self {
            has_initialized: initialization_type == InitializationType::PreInitialized,
            ..Self::default()
        }
    }
    ...
    for Simulation<WorldType /*, INITIALIZATION_TYPE*/>
{
    ...
    fn reset_last_completed_timestamp(&mut self, timestamp: Timestamp) {
        ...
        if timestamp < old_timestamp {
            ...
            for command in commands {
                log::trace!("Applying stale command after large timeskip {:?}", &command);
                self.world.apply_command(&command);
            }
        }
    }
